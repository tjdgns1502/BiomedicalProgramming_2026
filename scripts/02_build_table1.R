...
read_cycle <- function(cycle) {
  ...
  wt_var <- if (cycle == "P") "WTMECPRP" else "WTMEC2YR"
...
# ---- Combined MEC exam weight (2011-2012, 2013-2014, 2015-2016 = 2 yrs each;
# 2017-Mar2020 pre-pandemic = 3.2 yrs; total span = 9.2 yrs) ----
period_years <- c(G = 2, H = 2, I = 2, P = 3.2)
total_years <- sum(period_years)
all_data$WTCOMB <- all_data$WT2YR * (period_years[all_data$cycle] / total_years)
...
saveRDS(d, file.path("data", "analytic_sample.rds"))
message("Saved analytic_sample.rds")
