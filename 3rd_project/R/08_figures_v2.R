## 08_figures_v2.R --------------------------------------------------------
## v2 of 08_figures.R. Same data and seeds; three presentation defects fixed
## (see CHANGES_v2.md). Output goes to figs/v2/ so the v1 figures stay intact.
##   fig1: facet_wrap by (dataset, group) -- v1 was a 4x3 grid with 6 empty panels
##   fig2a: explicit facet order, caption names the panels instead of LEFT/RIGHT
##   fig5: 'published value' label moved off the curve
##
...
sv <- function(name, plot, w, h) {
  dir.create(file.path(FIG_DIR, "v2"), showWarnings = FALSE, recursive = TRUE)
  f <- file.path(FIG_DIR, "v2", name)
...
## one facet per (dataset, group) so that no panel is empty
q1$lab <- factor(paste0(q1$panel, "  |  ", q1$group),
                 levels = unique(paste0(q1$panel, "  |  ", q1$group)))
...
p1 <- ggplot(q1, aes(theoretical, sample)) +
  ...
  labs(title = "Group-wise normal Q-Q plots",
       ...
       caption = paste0(
         ...
         "The female triglyceride panel is stretched by a single 4,233 mg/dL observation; ",
         "axes are free per panel, so slopes are compared within a panel, not across panels."))
sv("fig1_qq_groupwise.png", p1, 11, 10)
...
vl <- data.frame(panel = factor(levels(res_df$panel), levels = levels(res_df$panel)),
                 x = obs_diff)
...
p2a <- ggplot(res_df, aes(value)) +
  ...
  labs(title = "What a p-value is, geometrically",
       ...
       caption = paste0(
         "Red line = observed statistic. PERMUTATION panel: the p-value is the tail area of the null distribution beyond the line.\n",
         "BOOTSTRAP panel: the distribution is centred at the observed value and shows the uncertainty of the estimate; the two panels answer different questions."))
...
p5 <- ggplot(fig5, aes(n, 100 * rate, colour = test)) +
  ...
  geom_text(data = paper_pts, aes(n, 100 * rate, label = "Fagerland's published value"),
            hjust = 1.08, vjust = -1.0, size = 2.9, colour = "black",
            inherit.aes = FALSE) +
...
saveRDS(list(nsub = nsub, obs_diff = obs_diff, p_perm = p_perm,
        ...
        file.path(RES_DIR, "08_figures_v2.rds"))
cat("\nall figures written to figs/v2/\n")
